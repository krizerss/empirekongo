...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('');
    if (strength < 2) {
      setError('Votre mot de passe est trop faible. Renforcez-le.');
      return;
    }
    if (password !== confirm) {
    ...
    <div className="min-h-screen bg-background flex">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="space-y-6">
            ...
            <h1 className="text-4xl font-extrabold text-foreground leading-tight">
              Créez un<br />
              <span className="gold-text">mot de passe</span><br />
              sécurisé
            </h1>
            <p className="text-muted-foreground text-base leading-relaxed max-w-sm">
              Choisissez un mot de passe fort pour protéger votre compte EmpireKongo.
            </p>
            <div className="space-y-3 pt-2">
              {requirements.map((req) => (
                <div key={req.label} className={`flex items-center gap-3 transition-opacity ${password ? '' : 'opacity-50'}`}>
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 transition-colors ${req.test(password) ? 'bg-green-500/20 border border-green-500/40' : 'bg-secondary border border-border'}`}>
                    {req.test(password) && <CheckIcon className="w-3 h-3 text-green-400" />}
                  </div>
                  <span className={`text-sm transition-colors ${req.test(password) ? 'text-green-400' : 'text-muted-foreground'}`}>{req.label}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="border-l-2 border-primary/50 pl-4">
            <p className="text-sm text-muted-foreground italic">&ldquo;Un bon mot de passe est votre première ligne de défense.&rdquo;</p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 lg:px-16">
        ...
        <div className="w-full max-w-md">
          <Link href="/auth/forgot-password" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-8">
          ...
          {success ? (
            <div className="space-y-6 text-center">
              ...
              <div>
                <h2 className="text-2xl font-extrabold text-foreground mb-2">Mot de passe mis à jour !</h2>
                <p className="text-muted-foreground text-sm">Votre mot de passe a été réinitialisé avec succès. Vous pouvez maintenant vous connecter.</p>
              </div>
            ...
            <>
              <div className="mb-8">
                ...
                <p className="text-muted-foreground text-sm">Choisissez un mot de passe fort et mémorable.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-foreground" htmlFor="password">Nouveau mot de passe</label>
                  ...
                  {password && (
                    <div className="space-y-1.5 pt-1">
                      ...
                      {strengthInfo && (
                        <p className={`text-xs font-semibold ${strengthInfo.textColor}`}>{strengthInfo.label}</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full bg-secondary border rounded-xl pl-10 pr-11 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 transition-colors ${
                        passwordsMismatch ? 'border-red-500/60 focus:border-red-500 focus:ring-red-500/20' : passwordsMatch ?'border-green-500/60 focus:border-green-500 focus:ring-green-500/20': 'border-border focus:border-primary focus:ring-primary/30'
                      }`}
                    />
                  ...
                  {passwordsMatch && <p className="text-xs text-green-400 flex items-center gap-1"><CheckIcon className="w-3 h-3" /> Les mots de passe correspondent</p>}
                  {passwordsMismatch && <p className="text-xs text-red-400">Les mots de passe ne correspondent pas</p>}
                </div>

                {/* Mobile requirements */}
                <div className="lg:hidden bg-secondary border border-border rounded-xl p-4 space-y-2">
                  <p className="text-xs font-semibold text-foreground mb-2">Exigences :</p>
                  {requirements.map((req) => (
                    <div key={req.label} className="flex items-center gap-2">
                      <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${req.test(password) ? 'bg-green-500/20' : 'bg-secondary border border-border'}`}>
                        {req.test(password) && <CheckIcon className="w-2.5 h-2.5 text-green-400" />}
                      </div>
                      <span className={`text-xs ${req.test(password) ? 'text-green-400' : 'text-muted-foreground'}`}>{req.label}</span>
                    </div>
                  ))}
                </div>
