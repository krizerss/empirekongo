    ...
    async function loadEnterprises() {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from('enterprises')
          .select('id,name,slug,category,description,city,logo_url,cover_url,verified,is_verified,created_at')
          ...
          <div className="max-w-7xl mx-auto">
            <div className="mb-6">
              ...
              <p className="text-sm text-muted-foreground">
                {loading ? 'Chargement des entreprises...' : `${filtered.length} fournisseurs trouvés sur EmpireKongo`}
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold transition-colors ${showFilters ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary border-border text-muted-foreground hover:text-foreground'}`}
              >
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          {loading ? (
            <div className="text-center py-20">
              <p className="text-foreground font-bold">Chargement des entreprises...</p>
              <p className="text-muted-foreground text-sm mt-1">Récupération des informations depuis EmpireKongo.</p>
            </div>
          ) : loadError ? (
            <div className="text-center py-20">
              <p className="text-foreground font-bold">{loadError}</p>
              <p className="text-muted-foreground text-sm mt-1">Vérifiez la connexion à Supabase puis rechargez la page.</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20">
                  ...
                  <div className="px-4 pt-2 pb-4 flex flex-col flex-1">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[10px] text-muted-foreground">{supplier.reviews || 0} avis</p>
                      </div>
                      <div className="w-px h-8 bg-border" />
                      <div className="text-center">
