        ...
        <div className="p-5">
          <div className="flex items-center gap-4 mb-5">
            <div className="w-14 h-14 rounded-2xl bg-primary/15 flex items-center justify-center shrink-0 overflow-hidden">
              {seller.logoUrl ? (
                <AppImage src={seller.logoUrl} alt={seller.name} width={56} height={56} className="w-full h-full object-cover" />
              ) : (
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-secondary rounded-xl">
              ...
              <div><p className="text-[10px] text-muted-foreground uppercase tracking-wide font-semibold">Localisation</p><p className="text-sm font-semibold text-foreground">{seller.city}</p></div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-secondary rounded-xl">
              <BuildingOfficeIcon className="w-4 h-4 text-primary shrink-0" />
              <div><p className="text-[10px] text-muted-foreground uppercase tracking-wide font-semibold">Secteur</p><p className="text-sm font-semibold text-foreground">{seller.category}</p></div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-secondary rounded-xl">
              <PhoneIcon className="w-4 h-4 text-primary shrink-0" />
              <div><p className="text-[10px] text-muted-foreground uppercase tracking-wide font-semibold">Téléphone</p><p className="text-sm font-semibold text-foreground">{seller.phone || '—'}</p></div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-secondary rounded-xl">
              <EnvelopeIcon className="w-4 h-4 text-primary shrink-0" />
              <div><p className="text-[10px] text-muted-foreground uppercase tracking-wide font-semibold">Email</p><p className="text-sm font-semibold text-foreground">{seller.email || '—'}</p></div>
            </div>
          </div>
          <button onClick={onContact} className="w-full mt-4 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-bold hover:bg-primary/90 transition-colors">Contacter le vendeur</button>
        </div>
      </div>
    </div>
  );
...
function OrderModal({ product, mode, onClose }: { product: OrderProduct; mode: 'reserve' | 'order'; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!Number.isInteger(parsedQuantity) || parsedQuantity < 1) {
      setErrorMessage('Veuillez saisir une quantité valide.');
      return;
    }
    ...
    try {
      ...
      if (userError || !user) throw new Error('Votre session a expiré. Veuillez vous reconnecter.');

      const { data: productRow, error: productError } = await supabase
        .from('products')
        .select('id,name,price,unit,vendor_id,enterprise_id,stock_status,stock_qty')
        .eq('id', product.id)
        .single();
      if (productError || !productRow) throw new Error('Produit introuvable.');
      if (productRow.vendor_id === user.id) throw new Error('Vous ne pouvez pas commander ou réserver votre propre produit.');
      if (productRow.stock_status === 'Rupture de stock') throw new Error('Ce produit est actuellement en rupture de stock.');
      if (productRow.stock_qty != null && parsedQuantity > productRow.stock_qty) throw new Error(`Quantité indisponible. Stock restant : ${productRow.stock_qty}.`);

      if (mode === 'reserve') {
        const { error } = await supabase.from('reservations').insert({
          buyer_id: user.id,
          seller_id: productRow.vendor_id,
        ...
        const { data: order, error: orderError } = await supabase
          ...
          .insert({
            buyer_id: user.id,
            seller_id: productRow.vendor_id,
            total_amount: unitPrice * parsedQuantity,
            currency: 'CDF',
            notes: note.trim(),
          })
          .select('id')
          .single();
        if (orderError || !order) throw orderError || new Error('Impossible de créer la commande.');
      ...
      console.error('Marketplace order/reservation:', err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl w-full max-w-sm shadow-2xl shadow-black/60" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="text-sm font-extrabold text-foreground">{mode === 'reserve' ? '📅 Réserver ce produit' : '🛒 Commander ce produit'}</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"><XMarkIcon className="w-4 h-4" /></button>
        </div>
        {submitted ? (
          <div className="p-6 text-center">
            <div className="w-14 h-14 rounded-full bg-green-400/15 flex items-center justify-center mx-auto mb-3"><span className="text-2xl">{mode === 'reserve' ? '📅' : '✅'}</span></div>
            <h3 className="text-base font-extrabold text-foreground mb-1">{mode === 'reserve' ? 'Réservation envoyée !' : 'Commande envoyée !'}</h3>
            <p className="text-xs text-muted-foreground mb-4">{mode === 'reserve' ? 'Le vendeur vous contactera pour confirmer la réservation.' : 'Le vendeur a reçu votre commande et vous contactera bientôt.'}</p>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            <div className="bg-secondary rounded-xl p-3"><p className="text-xs text-muted-foreground mb-0.5">Produit</p><p className="text-sm font-bold text-foreground">{product.name}</p><p className="text-xs text-primary font-semibold mt-0.5">{product.price} {product.unit}</p><p className="text-[10px] text-muted-foreground mt-1">Vendeur : {product.vendor}</p></div>
            <div className="flex flex-col gap-1.5"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Quantité</label><input type="number" min="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="bg-secondary border border-border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:border-primary/60 transition-colors" /></div>
            <div className="flex flex-col gap-1.5"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Note (optionnel)</label><textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder={mode === 'reserve' ? 'Date souhaitée, conditions...' : 'Instructions de livraison, précisions...'} rows={3} className="bg-secondary border border-border rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/60 transition-colors resize-none" /></div>
            {errorMessage && <p className="text-xs text-red-400">{errorMessage}</p>}
            <div className="flex gap-3 pt-1"><button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-border text-sm font-semibold text-muted-foreground hover:text-foreground hover:border-primary/40 transition-colors">Annuler</button><button type="submit" disabled={saving} className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-bold hover:bg-primary/90 transition-colors disabled:opacity-50">{saving ? 'Enregistrement...' : mode === 'reserve' ? 'Réserver' : 'Commander'}</button></div>
          </form>
        )}
      </div>
    </div>
...
function StarRating({ rating, size = 'sm' }: { rating: number; size?: 'sm' | 'md' }) {
  ...
  return <div className="flex items-center gap-0.5">{[1, 2, 3, 4, 5].map((s) => <StarSolid key={s} className={`${sz} ${s <= Math.round(rating) ? 'text-amber-400' : 'text-muted-foreground/30'}`} />)}</div>;
}

function ProductDetailModal({ product, onClose, onOrder, onReserve, onViewSeller }: { product: Product; onClose: () => void; onOrder: () => void; onReserve: () => void; onViewSeller: () => void }) {
  ...
  const stockColor = product.stock === 'En stock' ? 'text-green-400 bg-green-400/10 border-green-400/20' : product.stock === 'Stock limité' ? 'text-amber-400 bg-amber-400/10 border-amber-400/20' : 'text-red-400 bg-red-400/10 border-red-400/20';
  const stockDot = product.stock === 'En stock' ? 'bg-green-400' : product.stock === 'Stock limité' ? 'bg-amber-400' : 'bg-red-400';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/75 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto shadow-2xl shadow-black/70" onClick={(e) => e.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3.5 border-b border-border bg-card/95 backdrop-blur-sm"><button onClick={onClose} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"><ArrowLeftIcon className="w-3.5 h-3.5" />Retour au marketplace</button><span className="badge-gold text-[10px]">{product.category}</span></div>
        <div className="p-5 space-y-5">
          <div className="space-y-2">
            <div className="relative h-56 sm:h-72 rounded-xl overflow-hidden bg-secondary"><AppImage src={images[activeImg]?.src} alt={images[activeImg]?.alt || product.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, 672px" />{images.length > 1 && <><button onClick={() => setActiveImg((prev) => (prev - 1 + images.length) % images.length)} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 flex items-center justify-center text-white hover:bg-black/70 transition-colors"><ChevronLeftIcon className="w-4 h-4" /></button><button onClick={() => setActiveImg((prev) => (prev + 1) % images.length)} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/50 flex items-center justify-center text-white hover:bg-black/70 transition-colors"><ChevronRightIcon className="w-4 h-4" /></button><div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">{images.map((_, i) => <button key={i} onClick={() => setActiveImg(i)} className={`w-1.5 h-1.5 rounded-full transition-all ${i === activeImg ? 'bg-white w-4' : 'bg-white/50'}`} />)}</div></>}</div>
            {images.length > 1 && <div className="flex gap-2 overflow-x-auto pb-1">{images.map((img, i) => <button key={i} onClick={() => setActiveImg(i)} className={`relative w-14 h-14 rounded-lg overflow-hidden shrink-0 border-2 transition-all ${i === activeImg ? 'border-primary' : 'border-border hover:border-primary/40'}`}><AppImage src={img.src} alt={img.alt || product.name} fill className="object-cover" sizes="56px" /></button>)}</div>}
          </div>
          <div><h1 className="text-xl font-extrabold text-foreground leading-tight mb-2">{product.name}</h1><div className="flex items-center flex-wrap gap-3 mb-3"><div className="flex items-baseline gap-1"><span className="text-2xl font-extrabold text-primary">{product.price}</span><span className="text-sm text-muted-foreground">{product.unit}</span></div>{product.stock && <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${stockColor}`}><span className={`w-1.5 h-1.5 rounded-full ${stockDot}`} />{product.stock}{product.stock === 'Stock limité' && product.stockQty ? <span className="opacity-70">({product.stockQty} restants)</span> : null}</span>}</div>{product.rating != null && <div className="flex items-center gap-2"><StarRating rating={product.rating} size="md" /><span className="text-sm font-bold text-foreground">{product.rating.toFixed(1)}</span><span className="text-xs text-muted-foreground">({product.reviewCount ?? 0} avis)</span></div>}</div>
          ...
          {product.reviews && product.reviews.length > 0 && <div><div className="flex items-center justify-between mb-3"><h2 className="text-sm font-extrabold text-foreground uppercase tracking-wide">Avis clients</h2><div className="flex items-center gap-1.5"><StarRating rating={product.rating || 0} /><span className="text-xs text-muted-foreground">{product.reviewCount ?? 0} avis</span></div></div><div className="space-y-3">{product.reviews.map((review, idx) => <div key={idx} className="p-3.5 bg-secondary rounded-xl"><div className="flex items-start justify-between gap-2 mb-1.5"><div><p className="text-xs font-bold text-foreground">{review.author}</p><p className="text-[10px] text-muted-foreground">{review.date}</p></div><StarRating rating={review.rating} /></div><p className="text-xs text-muted-foreground leading-relaxed">{review.comment}</p></div>)}</div></div>}
...
export default function MarketplaceContent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await supabase
        .from('products')
        .select('id,name,description,price,unit,category,city,image_url,alt_text,stock_status,stock_qty,rating,review_count,vendor_id,vendor_name,vendor_type,vendor_phone,vendor_email,vendor_city,vendor_verified,vendor_since,enterprise_id,created_at,enterprises:enterprise_id(name,logo_url,cover_url,is_verified,founded_year,city)')
        .eq('is_active', true)
      ...
      if (error) {
        ...
        setProducts([]);
        setLoadError('Impossible de charger les produits pour le moment.');
        setLoading(false);
        return;
      }
      ...
      setProducts(rows.map((r) => ({
        id: r.id,
        name: r.name,
        vendor: r.vendor_name || r.enterprises?.name || 'Vendeur',
        vendorType: r.vendor_type || 'Entreprise',
        vendorPhone: r.vendor_phone || '',
        vendorEmail: r.vendor_email || '',
        vendorCity: r.vendor_city || r.enterprises?.city || r.city || '',
        vendorVerified: Boolean(r.vendor_verified || r.enterprises?.is_verified),
        price: r.price,
        unit: r.unit ? (r.unit.startsWith('/') ? r.unit : `/ ${r.unit}`) : '',
        category: r.category || 'Autres',
        city: r.city || r.vendor_city || '',
        image: r.image_url || '',
        alt: r.alt_text || r.name,
        description: r.description || undefined,
        stock: r.stock_status || 'En stock',
        stockQty: r.stock_qty ?? 0,
        rating: r.rating == null ? undefined : Number(r.rating),
        reviewCount: r.review_count ?? 0,
        vendorSince: r.vendor_since || (r.enterprises?.founded_year ? String(r.enterprises.founded_year) : ''),
        vendorProducts: r.vendor_id ? counts.get(r.vendor_id) || 0 : undefined,
        vendorLogo: r.enterprises?.logo_url || undefined,
        vendorCover: r.enterprises?.cover_url || undefined,
        enterpriseId: r.enterprise_id || undefined,
      })));
      setLoading(false);
    };
  ...
  const allCategories = useMemo(() => {
    ...
    return [{ label: 'Toutes catégories', count: products.length }, ...Array.from(m.entries()).sort(([a], [b]) => a.localeCompare(b, 'fr')).map(([label, count]) => ({ label, count }))];
  }, [products]);
  ...
  const handleProtectedAction = (action: 'order' | 'reserve' | 'message' | 'contact', fn: () => void) => {
    if (!isLoggedIn) setAuthGuard(action);
    else fn();
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex gap-6">
        <aside className="hidden lg:flex flex-col gap-6 w-56 shrink-0">
          <div className="bg-card border border-border rounded-xl overflow-hidden"><div className="px-4 py-3 border-b border-border"><h3 className="font-bold text-sm text-foreground">Catégories</h3></div><div className="p-2">{allCategories.map((cat) => <button key={cat.label} onClick={() => setSelectedCategory(cat.label)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all ${selectedCategory === cat.label ? 'bg-primary/15 text-primary font-semibold' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'}`}><span>{cat.label}</span>{cat.count > 0 && <span className={`text-[11px] ${selectedCategory === cat.label ? 'text-primary' : 'text-muted-foreground'}`}>{cat.count}</span>}</button>)}</div></div>
          <div className="bg-card border border-border rounded-xl overflow-hidden"><div className="px-4 py-3 border-b border-border"><h3 className="font-bold text-sm text-foreground">Localisation</h3></div><div className="p-2">{allCities.map((city) => <button key={city} onClick={() => setSelectedCity(city)} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all ${selectedCity === city ? 'bg-primary/15 text-primary font-semibold' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'}`}><MapPinIcon className="w-3.5 h-3.5" /><span>{city}</span></button>)}</div></div>
        </aside>
        <div className="flex-1 min-w-0">
          ...
          {loading ? <div className="text-center py-20 text-muted-foreground"><p className="text-sm">Chargement des produits...</p></div> : loadError ? <div className="text-center py-20 text-muted-foreground"><p className="text-lg font-semibold mb-2">Marketplace indisponible</p><p className="text-sm">{loadError}</p></div> : <>
