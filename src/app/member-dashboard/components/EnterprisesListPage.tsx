    ...
    async function loadEnterprises() {
      ...
      try {
        ...
        const { data, error: queryError } = await supabase
          .from('enterprises')
          .select(
            'id,name,logo_url,cover_url,category,city,address,description,phone,email,website,employee_count,founded_year,is_verified'
          )
          .eq('is_active', true)
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-muted-foreground">
          ...
          <p className="text-sm text-muted-foreground/60">
            {enterprises.length === 0
              ? "Aucune entreprise active n'est encore enregistrée."
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((enterprise) => (
            <div
              key={enterprise.id}
              className="bg-card border border-border rounded-xl overflow-hidden hover:border-primary/40 hover:shadow-lg transition-all duration-200 group"
            >
              <div className="relative h-28 overflow-hidden">
                <AppImage
                  src={enterprise.cover_url || FALLBACK_COVER}
              ...
              <div className="px-4 pb-4 -mt-7 relative">
                <div className="flex items-end gap-3 mb-3">
                  <div className="w-14 h-14 rounded-xl overflow-hidden border-3 border-card bg-card shadow-lg shrink-0">
                    <AppImage
                      src={enterprise.logo_url || FALLBACK_LOGO}
                  ...
                  <div className="flex-1 min-w-0 pb-1">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <span className="text-[10px] text-muted-foreground truncate">
                        {enterprise.category || 'Non catégorisée'}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 text-muted-foreground mb-2">
                  ...
                  <span className="text-[11px] truncate">
                    {enterprise.city || enterprise.address || 'Localisation non renseignée'}
                  </span>
                </div>

                <p className="text-[11px] text-muted-foreground leading-relaxed line-clamp-2 mb-3 min-h-[32px]">
                  {enterprise.description || 'Aucune description disponible.'}
                </p>
                ...
                <div className="flex items-center gap-3 mb-3 text-[10px] text-muted-foreground">
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      {enterprise.employee_count ?? 0} employé{(enterprise.employee_count ?? 0) !== 1 ? 's' : ''}
                    </span>
                  </div>
                  {enterprise.founded_year && (
                    <div className="flex items-center gap-1">
                      <BuildingOfficeIcon className="w-3 h-3" />
                      <span>Depuis {enterprise.founded_year}</span>
                    </div>
                  )}
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    if (enterprise.slug) {
                      window.location.href = `/entreprises/${enterprise.slug}`;
                    }
                  }}
                  className="w-full py-2 rounded-xl gold-gradient text-primary-foreground text-xs font-bold hover:opacity-90 transition-opacity shadow-sm"
                >
                  Voir l'entreprise
                </button>
              </div>
