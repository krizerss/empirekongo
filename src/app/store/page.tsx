...
interface Product {
  id: string; name: string; vendor: string; vendorVerified: boolean; price: string; priceNum: number; unit: string; category: string; city: string; image: string; alt: string; rating: number; reviews: number; isPromo?: boolean; promoLabel?: string; isSponsored?: boolean; stock: number;
}

const categoryDefinitions = [
{ label: 'Toutes', icon: '🏪' }, { label: 'Agriculture', icon: '🌾' }, { label: 'Élevage', icon: '🐄' }, { label: 'Agroalimentaire', icon: '🥘' }, { label: 'Énergie', icon: '⚡' }, { label: 'BTP & Matériaux', icon: '🏗️' }, { label: 'Mode & Beauté', icon: '👗' }, { label: 'Technologie', icon: '💻' }];
const ITEMS_PER_PAGE = 12;
const sortOptions = [{ value: 'popular', label: 'Plus populaires' }, { value: 'price_asc', label: 'Prix croissant' }, { value: 'price_desc', label: 'Prix décroissant' }, { value: 'rating', label: 'Mieux notés' }, { value: 'newest', label: 'Nouveautés' }];
...
export default function StorePage() {
  ...
  const mapProduct = (row: any): Product => {
    ...
    return { id: String(row.id), name: row.name, vendor: row.vendor_name || 'Fournisseur EmpireKongo', vendorVerified: row.vendor_verified === true, price: priceText, priceNum: numericPrice, unit: row.unit || '', category: row.category || 'Autres', city: row.city || row.vendor_city || 'Non précisée', image: row.image_url || '/assets/images/placeholder.png', alt: row.alt_text || row.name, rating: Number(row.rating) || 0, reviews: Number(row.review_count) || 0, stock: Number(row.stock_qty) || 0 };
  };
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      const { data, error } = await supabase.from('products').select('id,name,description,price,unit,category,city,image_url,alt_text,stock_status,stock_qty,rating,review_count,vendor_id,vendor_name,vendor_type,vendor_city,vendor_verified,is_active,created_at,updated_at').eq('is_active', true).order('created_at', { ascending: false });
      if (!active) return;
      if (error) { setLoadError('Impossible de charger les produits.'); setProducts([]); } else setProducts((data || []).map(mapProduct));
      setLoading(false);
    };
    loadProducts();
    const channel = supabase.channel('store-products').on('postgres_changes', { event: '*', schema: 'public', table: 'products' }, () => loadProducts()).subscribe();
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === 'price_asc') list.sort((a, b) => a.priceNum - b.priceNum); else if (sortBy === 'price_desc') list.sort((a, b) => b.priceNum - a.priceNum); else if (sortBy === 'rating') list.sort((a, b) => b.rating - a.rating);
    return list;
  ...
  useEffect(() => { setCurrentPage(1); }, [search, selectedCategory, selectedCity, sortBy, priceRange, onlyPromo, onlyVerified]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-16">
        <div className="bg-card border-b border-border px-4 py-8"><div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6"><div><h1 className="text-2xl font-extrabold text-foreground">Store EmpireKongo</h1><p className="text-sm text-muted-foreground mt-1">{loading ? 'Chargement des produits...' : `${filtered.length} produits disponibles`}</p></div><div className="flex items-center gap-2"><button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}><Squares2X2Icon className="w-4 h-4" /></button><button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}><ListBulletIcon className="w-4 h-4" /></button></div></div>
          <div className="flex gap-3"><div className="relative flex-1"><MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un produit, un fournisseur..." className="w-full pl-10 pr-4 py-2.5 bg-secondary border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-primary/60 transition-colors" />{search && <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2"><XMarkIcon className="w-4 h-4 text-muted-foreground hover:text-foreground" /></button>}</div><button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-semibold transition-colors ${showFilters ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary border-border text-muted-foreground hover:text-foreground'}`}><AdjustmentsHorizontalIcon className="w-4 h-4" />Filtres</button><select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-3 py-2.5 bg-secondary border border-border rounded-xl text-sm text-foreground focus:outline-none focus:border-primary/60 transition-colors">{sortOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
        </div></div>
        <div className="max-w-7xl mx-auto px-4 py-6"><div className="flex gap-6">
          {showFilters && <aside className="w-64 shrink-0 space-y-6"><div className="bg-card border border-border rounded-xl p-4"><h3 className="text-sm font-bold text-foreground mb-3">Catégories</h3><div className="space-y-1">{categories.map((cat) => <button key={cat.label} onClick={() => setSelectedCategory(cat.label)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${selectedCategory === cat.label ? 'bg-primary/15 text-primary font-semibold' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'}`}><span className="flex items-center gap-2"><span>{cat.icon}</span>{cat.label}</span><span className="text-xs opacity-60">{cat.count}</span></button>)}</div></div><div className="bg-card border border-border rounded-xl p-4"><h3 className="text-sm font-bold text-foreground mb-3">Ville</h3><div className="space-y-1">{cities.map((city) => <button key={city} onClick={() => setSelectedCity(city)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${selectedCity === city ? 'bg-primary/15 text-primary font-semibold' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'}`}>{city}</button>)}</div></div><div className="bg-card border border-border rounded-xl p-4 space-y-3"><h3 className="text-sm font-bold text-foreground mb-1">Options</h3><label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={onlyPromo} onChange={(e) => setOnlyPromo(e.target.checked)} className="w-4 h-4 accent-primary" /><span className="text-sm text-muted-foreground">Promotions uniquement</span></label><label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={onlyVerified} onChange={(e) => setOnlyVerified(e.target.checked)} className="w-4 h-4 accent-primary" /><span className="text-sm text-muted-foreground">Fournisseurs vérifiés</span></label></div></aside>}
      ...
      </main><Footer />{showAuthGuard && <AuthGuardModal action="order" onClose={() => setShowAuthGuard(false)} />}
    </div>);
}
