...
export default function StorePage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        ...
        if (error) {
          console.log('Store products load error:', error.message);
          setAllProducts([]);
        } else {
          const mapped: Product[] = (data || []).map((p: any) => {
            // Parse price number from price string (e.g. "25,000 FC" → 25000)
            const priceNum = parseFloat(String(p.price_num || p.price || '0').replace(/[^0-9.]/g, '')) || 0;
            return {
              id: p.id,
              name: p.name,
        ...
        console.log('Store products fetch failed:', err);
        setAllProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [allProducts, search, selectedCategory, selectedCity, sortBy, priceRange, onlyPromo, onlyVerified]);
